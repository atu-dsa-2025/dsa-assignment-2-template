...
class Booking:
    ...
    def cancel_booking(self, booking_id):
        ...
        """
        # Implement cancel_booking handling head, middle, and tail deletions.
            ...
            if current.booking_id == booking_id:
                ...
                if current.prev is None:
                    ...
                    if self.head is not None:
                        ...
                        # list became empty; update tail if present
    ...
    def find_and_swap(self, id1, id2):
        ...
        # Find the two nodes with the given IDs
        node1 = None
        node2 = None
        current = getattr(self, "head", None)
        while current:
        ...
        # Answer: swapping data is preferred because it avoids the complexity and potential errors of relinking pointers in a doubly linked list, which can be error-prone and may lead to broken links if not handled carefully. Swapping data is simpler and safer when the goal is just to exchange the contents of two nodes.
        if node1 and node2:
...
class RouteHistory:
    ...
    def __init__(self):
        ...
        self.routes = []  # Internal list acting as a stack
    
    def record_route(self, route):
        """
        Record a new route (push onto stack).
        
        Args:
            route (str): The route name/description to record.
    ...
    def undo(self):
        """
        Undo the last route change (pop from stack).
        
        Returns:
            str: The route that was undone, or None if no history.
    ...
    def current_route(self):
        """
        Peek at the current route without removing it.
        
        Returns:
            str: The current route, or None if no routes recorded.
    ...
    def is_empty(self):
        """
        Check if there are any routes in history.
    ...
    def history_count(self):
        """
        Get the number of routes in history.
        
        Returns:
            int: Number of recorded routes.
    ...
    def print_history(self):
        """
        Print all routes in the history (top to bottom).
        
        Complexity: O(N)
        """
        print("Route History (top to bottom):")
        for i in range(len(self.routes) - 1, -1, -1):
            print(f"  {i+1}. {self.routes[i]}")
        if self.is_empty():
            print("  (empty)")
        pass
