...
def d1_explanation():
    ...
    return """
    Hash collision definition: (Occur when two different keys produce the same hash value, leading to a conflict in the hash table.)

    Chaining:
        Description: (In Chaining, each bucket of the hash table contains a linked list (or another data structure) of all elements that hash to the same index. When a collision occurs, the new element is simply added to the list at that index.)
        Worst-case lookup: O(N) (In the worst case, all elements hash to the same index, resulting in a linked list of length N, leading to O(N) lookup time.)

    Open Addressing / Linear Probing:
        Description: (In Open Addressing with Linear Probing, when a collision occurs, the algorithm searches for the next available slot in the hash table sequentially. The search wraps around to the beginning if necessary.)
...
def grade_frequency_report(results):
    ...
    # Initialize the frequency dictionary with all grades set to 0
    freq = {'A': 0, 'B': 0, 'C': 0, 'D': 0, 'F': 0}
    # Iterate through the results and count each grade
...
def find_students_with_grade(results, grade):
    ...
    Why not O(N)?  (Although we use a hash table for grouping, the final step of sorting the list of student IDs takes O(N log N) time.)
    """
    # TODO: Implement find_students_with_grade.
    # Use a list to collect student IDs with the specified grade
    student_ids = [student_id for student_id, g in results if g == grade]
    # Sort the list of student IDs before returning
...
def insert(root, student_id, grade_score):
    ...
    # If grade_score == root.grade_score, do not insert (ignore duplicates)
    return root
    pass
...
def search(root, grade_score):
    ...
    When is H = O(log N)?  (Occurs when the BST is balanced, meaning the height of the tree is logarithmic relative to the number of nodes.)
    When is H = O(N)?      (Occurs when the BST is completely unbalanced, forming a linked list.)
    """
...
def find_range(root, low, high):
    ...
    if low <= root.grade_score <= high:
        student_ids.append(root.student_id)

    if low < root.grade_score:
        student_ids.extend(find_range(root.left, low, high))

    if root.grade_score < high:
        student_ids.extend(find_range(root.right, low, high))
